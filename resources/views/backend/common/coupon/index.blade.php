...
function createNew() {
    ...
    openCreateModal("{{ route(\Request::segment(1) . '.coupon.create') }}");
}
...
function submitData() {
    const name = $('#name').val();
    upsert(
        "{{ route(\Request::segment(1) . '.coupon.store') }}", "POST", {
            'name': name,
        }
    );
    getData();
...
function changeStatus(id) {
    const url = "{{ route(Request::segment(1) . '.coupon.status.change', ['id' => 'id']) }}".replace(
        'id', id);
    upsert(url, "PUT", {
        _method: 'PUT'
    });
...
function editModal(id) {
    const url = "{{ route(Request::segment(1) . '.coupon.edit', ['id' => 'id']) }}".replace('id', id);
    openEditModal(url);
}
...
function updateData(id) {
    const name = $('#name').val();
    upsert("{{ route(\Request::segment(1) . '.coupon.update') }}", "POST", {
        'id': id,
        'name': name,
    });
    getData();
...
function getData() {
    dataTable("{{ route(Request::segment(1) . '.coupon.show') }}", {}, [{
            data: 'id',
            name: 'id'
        },
        {
