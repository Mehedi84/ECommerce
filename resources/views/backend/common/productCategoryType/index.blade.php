...
<script>
    ...
    function createNew() {
        ...
        openCreateModal("{{ route(\Request::segment(1) . '.product.category.type.create') }}");
    }
    ...
    function submitData() {
        const name = $('#name').val();
        upsert(
            "{{ route(\Request::segment(1) . '.product.category.type.store') }}", "POST", {
                'name': name,
            }
        );
        getData();
    ...
    function changeStatus(id) {
        const url = "{{ route(Request::segment(1) . '.product.category.type.status.change', ['id' => 'id']) }}".replace(
            'id', id);
        upsert(url, "PUT", {
            _method: 'PUT'
        });
    ...
    function editModal(id) {
        const url = "{{ route(Request::segment(1) . '.product.category.type.edit', ['id' => 'id']) }}".replace('id', id);
        openEditModal(url);
    }
    ...
    function updateData(id) {
        const name = $('#name').val();
        upsert("{{ route(\Request::segment(1) . '.product.category.type.update') }}", "POST", {
            'id': id,
            'name': name,
        });
        getData();
    ...
    function getData() {
        dataTable("{{ route(Request::segment(1) . '.product.category.type.show') }}", {}, [{
                data: 'id',
                name: 'id'
            },
            {
