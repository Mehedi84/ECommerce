...
function createNew() {
    ...
    openCreateModal("{{ route(\Request::segment(1) . '.product.category.create') }}");
}
...
function submitData() {
    const name = $('#name').val();
    upsert(
        "{{ route(\Request::segment(1) . '.product.category.store') }}", "POST", {
            'name': name,
        }
    );
    getData();
...
function changeStatus(id) {
    const url = "{{ route(Request::segment(1) . '.product.category.status.change', ['id' => 'id']) }}".replace(
        'id', id);
    upsert(url, "PUT", {
        _method: 'PUT'
    });
...
function editModal(id) {
    const url = "{{ route(Request::segment(1) . '.product.category.edit', ['id' => 'id']) }}".replace('id', id);
    openEditModal(url);
}
...
function updateData(id) {
    const name = $('#name').val();
    upsert("{{ route(\Request::segment(1) . '.product.category.update') }}", "POST", {
        'id': id,
        'name': name,
    });
    getData();
...
function getData() {
    dataTable("{{ route(Request::segment(1) . '.product.category.show') }}", {}, [{
            data: 'id',
            name: 'id'
        },
        {
